...
def plot_auc_scores(auc_scores, N_features, N_samples):
    ...
    plt.title('Madelon Dataset. AUC Scores for {} randomly selected features'.format(N_features))
    ...
    plt.draw()
    plt.hold(False)
    fig1.savefig('plots/madelon/RANDOM_auc-{}features_{}samples.png'.format(N_features, N_samples), dpi=100)
...
n = n_features_start
comparator = AUCComparator(data, target)
while n < n_features_end:
    ...
    for i in range(N_samples):
        ...
        auc_scores.append(comparator.get_mean_score(all_features[:n]))
